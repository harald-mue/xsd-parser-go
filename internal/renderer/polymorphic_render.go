...
func renderCustomXML(buf *bytes.Buffer, file generator.File, typ generator.TypeDecl) {
	...
	for _, field := range c.Fields {
		...
		fmt.Fprintln(buf, "\t\t\t\txsiTypeValue := entry.XSIType.Local")
		fmt.Fprintln(buf, "\t\t\t\tif entry.XSIType.Namespace != \"\" {")
		fmt.Fprintln(buf, "\t\t\t\t\tif xsiPrefix := namespacePrefixes[entry.XSIType.Namespace]; xsiPrefix != \"\" {")
		fmt.Fprintln(buf, "\t\t\t\t\t\txsiTypeValue = xsiPrefix + \":\" + entry.XSIType.Local")
		fmt.Fprintln(buf, "\t\t\t\t\t}")
		fmt.Fprintln(buf, "\t\t\t\t}")
		fmt.Fprintln(buf, "\t\t\t\tchild.Attr = append(child.Attr, xml.Attr{Name: xml.Name{Local: \"xsi:type\"}, Value: xsiTypeValue})")
		fmt.Fprintln(buf, "\t\t\t}")
		fmt.Fprintln(buf, "\t\t\tif err := e.EncodeElement(v, child); err != nil {")
		fmt.Fprintln(buf, "\t\t\t\treturn err")
		fmt.Fprintln(buf, "\t\t\t}")
